...
class ModeleCommande extends connexion {
    ...
    public function verifierRoleBarman($login, $bar_id) {
        $req = self::$bdd->prepare("
            SELECT COUNT(*)
            FROM role
            WHERE login_utilisateur = :login
            AND bar_associe = :bar_id
            AND role_bar = 'barman'
        ");
    ...
    public function getClientsBuvette($bar_id) {
        $req = self::$bdd->prepare("
            SELECT u.login, u.solde
            FROM utilisateur u
            JOIN role r ON u.login = r.login_utilisateur
            WHERE r.bar_associe = :bar_id
            AND r.role_bar = 'client'
            ORDER BY u.login
        ");
        $req->bindParam(':bar_id', $bar_id);
        $req->execute();
    ...
    public function getSoldeClient($login) {
    ...
    public function getStockProduit($id_produit, $bar_id) {
        $req = self::$bdd->prepare("
            SELECT quantite
            FROM stock
    ...
    public function creerCommande($login_client, $bar_id) {
        $req = self::$bdd->prepare("
            INSERT INTO commande (date_commande, login_utilisateur, bar_associe)
            VALUES (CURDATE(), :login, :bar_id)
        ");
        $req->bindParam(':login', $login_client);
        $req->bindParam(':bar_id', $bar_id);
        $req->execute();
        return self::$bdd->lastInsertId();
    }

    public function ajouterProduitCommande($id_commande, $id_produit, $quantite, $prix) {
        $req = self::$bdd->prepare("
    ...
    public function debiterClient($login, $montant) {
        $req = self::$bdd->prepare("
            UPDATE utilisateur
